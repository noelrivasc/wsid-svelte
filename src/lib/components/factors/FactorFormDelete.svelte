...
<form method="POST" action="?/deleteFactor" use:enhance={submit} class="flex flex-col gap-4">
	<input type="hidden" name="id" value={factor.id} />

	<p class="text-body">
		Are you sure you want to delete factor <span class="font-semibold">{factor.title}</span>?
	</p>
